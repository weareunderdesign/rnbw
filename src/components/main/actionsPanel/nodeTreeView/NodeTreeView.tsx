...
export default function NodeTreeView(props: NodeTreeViewProps) {
    ...
    (nodeType: string) => {
      ...
      const newNode: TNode = {
        ...
        data: {
          ...
          data: "",
          attribs: {
            [NodeInAppAttribName]: String(nodeMaxUid + 1) as TNodeUid,
          },
  ...
  const cb_removeNode = useCallback(
    (uids: TNodeUid[]) => {
      ...
      if (allow) {
        ...
        setEvent({
          type: "remove-node",
          param: [uids, res.deletedUids, res.lastNodeUid],
        });
  ...
  const cb_groupNode = useCallback(
    (_uids: TNodeUid[], targetUid: TNodeUid, position: number) => {
      let Content: string = "";
      ...
      if (_tree) {
        ...
        for (let x in _tree) {
          ...
          if (x === "ROOT") {
            _tree[x].uid = String(Number(tmpMaxUid) + 1);
            _tree[x].parentUid =
              targetUid !== "ROOT" && _parent !== undefined
                ? nodeTree[targetUid].parentUid
                : "ROOT";
            _tree[x].name = newNode.name;
            _tree[x].data.type = "tag";
            _tree[x].data.name = newNode.name;
  ...
  const cb_unGroupNode = useCallback(
    (_uids: TNodeUid[], targetUid: TNodeUid) => {
      let uids: TNodeUid[] = [];
      for (let x in _uids) {
        if (
          !(nodeTree[_uids[x]].parentUid === RootNodeUid) &&
          nodeTree[_uids[x]].name !== "br" &&
          nodeTree[_uids[x]].name !== "script" &&
          nodeTree[_uids[x]].name !== "style"
        ) {
          uids.push(_uids[x]);
        }
      }
  ...
  const onGroup = useCallback(() => {
    ...
    for (let x in validNodeTree) {
      ...
      if (nodeData && nodeData.type === "tag" && nodeData.name === "body") {
        ...
        while (_parent.parentUid !== "ROOT") {
          if (
            (_parent.data as THtmlNodeData).type === "tag" &&
            ((_parent.data as THtmlNodeData).name === "pre" ||
              (_parent.data as THtmlNodeData).name === "code")
          ) {
            break;
          }
    ...
    const childIndex = getNodeChildIndex(
      validNodeTree[validNodeTree[_focusedItem].parentUid as TNodeUid],
      validNodeTree[_focusedItem],
    );
