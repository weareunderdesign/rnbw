...
export const validateUids = (uids: TUid[], targetUid ?: TUid) => {
  if (targetUid !== undefined) {
    uids = uids.filter((uid) => { return uid.startsWith(targetUid) !== true})
  }
  ...
  while (uids.length) {
    ...
    result.push(uid)
    uids = uids.filter((curUid) => { return curUid.startsWith(uid) !== true });
  }
  return result;
}